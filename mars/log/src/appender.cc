...
std::string appender_getLastFilepath()
{
    struct timeval tv;
    gettimeofday(&tv, NULL);
    
    std::string logfilenameprefix = __make_logfilenameprefix(tv, sg_logfileprefix.c_str());
    
    std::vector<std::string> filename_vec;
    __get_filenames_by_prefix(sg_logdir, logfilenameprefix, LOG_EXT, filename_vec);
    if (!sg_cache_logdir.empty()) {
        __get_filenames_by_prefix(sg_cache_logdir, logfilenameprefix, LOG_EXT, filename_vec);
    }
